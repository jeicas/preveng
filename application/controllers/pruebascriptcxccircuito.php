...
class Pruebascriptcxccircuito extends CI_Controller {
    ...
    public function index() {
        $circuito = $this->prueba_model->verificartipousuario();
                    $i=1;
        if ($circuito->num_rows > 0) {
            ...
            foreach ($circuito->result_array() as $row) {
                ...
                switch ($row['tipo']) {
                    case 2:{
                        $this->resmetacxcestados($row['us'], $i);
                       $i++;
                        break;
    ...
    public function resmetacxcestados($idus,$i) {
        print_r('<br>');
        $data = $this->prueba_model->verificardatos($idus);
        print_r('<b> Entrada Nro '.$i.'</b>');            
        print_r(' cuanta data trae '.$data->num_rows. ' ');
        if ($data->num_rows > 0) {
