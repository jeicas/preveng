...
class Prueba_model extends CI_Model {
  ...
  public function verificartipousuario() {
    ...
    public function verificarcircuitousuario($tipo) {
        ...
        public function verificardatos($idus) {
           /* print_r(" SELECT persona.email as correo,
                                          persona.email,persona.nombre as nombrecord,
                                          persona.apellido 
                                   FROM persona,usuario
                                   WHERE  persona.cedula=usuario.cedula AND usuario.tipousuario=2 AND usuario.id=$idus ");
            */
    ...
    public function verificartotalcircuito($idcircuito) {
        $query = $this->db->query("SELECT count(distinct planilla.cedula) as cantidad,
                                   ...
                                   from planilla
                                     ...
                                     inner join comunidad on comunidad.id=planilla.comunidad
                                     inner join parroquia on comunidad.parroquia=parroquia.id_parroquia and parroquia.id_parroquia=110205
                                     ...
                                     inner join municipio on parroquia.id_municipio=municipio.id_municipio 
                                      group by parroquia.id_parroquia ");
        return $query;
    }

    public function verificartotalmunicipio($idcircuito, $idmunicipio) {
       $fecha=date('Y-m-d', strtotime('-1 day'));
        $query = $this->db->query("SELECT count(distinct planilla.cedula) as cantidadmuni,municipio.nombre as municipio,centro_votacion.circuito from planilla
	INNER JOIN tipocompromiso ON planilla.tipocompromiso = tipocompromiso.id
        INNER JOIN tipoplanilla ON tipocompromiso.tipoplanilla = tipoplanilla.id AND tipoplanilla.id =1	
        inner join comunidad on comunidad.id=planilla.comunidad
        inner join parroquia on comunidad.parroquia=parroquia.id_parroquia 
        inner join centro_votacion on centro_votacion.parroquia=parroquia.id_parroquia 
        inner join circuito on centro_votacion.circuito=circuito.id and circuito.id=$idcircuito
       ...
       public function verificartotalmunicipioTodas($idcircuito, $idmunicipio) {
        ...
        $query = $this->db->query("SELECT count(distinct planilla.cedula) as todoMun,municipio.nombre as municipio,centro_votacion.circuito from planilla
		INNER JOIN tipocompromiso ON planilla.tipocompromiso = tipocompromiso.id
        INNER JOIN tipoplanilla ON tipocompromiso.tipoplanilla = tipoplanilla.id AND tipoplanilla.id =1			
        inner join comunidad on comunidad.id=planilla.comunidad
        inner join parroquia on comunidad.parroquia=parroquia.id_parroquia 
     ...
     $fecha=date('Y-m-d', strtotime('-1 day'));
        $query = $this->db->query("SELECT count(DISTINCT planilla.cedula) as cantidadtotal from planilla
	INNER JOIN tipocompromiso ON planilla.tipocompromiso = tipocompromiso.id
        INNER JOIN tipoplanilla ON tipocompromiso.tipoplanilla = tipoplanilla.id AND tipoplanilla.id =1			
        ...
        $query = $this->db->query("SELECT count(distinct planilla.cedula) as cantidadtotal
                                   from planilla
                                     INNER JOIN tipocompromiso ON planilla.tipocompromiso = tipocompromiso.id
                                     INNER JOIN tipoplanilla ON tipocompromiso.tipoplanilla = tipoplanilla.id AND tipoplanilla.id =1
                                     inner join comunidad on comunidad.id=planilla.comunidad
    ...
    public function verificartotalparroquia($idcircuito, $idmunicipio, $idparroquia) {
      ...
      $fecha=date('Y-m-d', strtotime('-1 day'));
        $query = $this->db->query("SELECT count(distinct planilla.cedula) as cantidadparri,
                                   municipio.nombre as municipio,
                                   centro_votacion.circuito, 
		...
		inner join comunidad on comunidad.id=planilla.comunidad
        inner join parroquia on comunidad.parroquia=parroquia.id_parroquia 
        inner join centro_votacion on centro_votacion.parroquia=parroquia.id_parroquia and parroquia.id_parroquia =$idparroquia
        inner join circuito on centro_votacion.circuito=circuito.id and circuito.id=$idcircuito
        inner join municipio on parroquia.id_municipio=municipio.id_municipio and  municipio.id_municipio=$idmunicipio 
       ...
       public function verificartotalparroquiatodas($idcircuito, $idmunicipio, $idparroquia) {
        ...
        $query = $this->db->query("SELECT count(distinct planilla.cedula) as todosParr,
                                   municipio.nombre as municipio,
                                   centro_votacion.circuito, 
                                   meta_parroquia.meta_diaria,  
                                   meta_parroquia.total_contacto 
