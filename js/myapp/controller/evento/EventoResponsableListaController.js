...
Ext.define('myapp.controller.evento.EventoResponsableListaController', {
    ...
    refs: [
        ...
        {
            ref: 'WinAsignarEncargado',
            selector: 'winAsignarEncargado'
        },
        {
            ref: 'ListaAsignarUsuarioEncargado',
            selector: 'listaAsignarUsuarioEncargado'
        },
        {
    ...
    init: function (application) {
        this.control({
            ...
            "listaAsignarUsuarioEncargado button[name=btnGuardar]": {
    ...
    onClickNuevoResponsable: function (record, item, index, e, eOpts) {
        ...
        if (item.data.nombrecompleto == '<font color=#FF0000> POR ASIGNAR </font>') {
            if (item.data.estatus == 'Sin Plan' || item.data.estatus == 'Pendiente') {
                nuevorespo = true;
                win = Ext.create('myapp.view.actividad.WinAsignarEncargado');
                win.setTitle("Asignar Responsable");
            } else {
                Ext.MessageBox.show({title: 'Informaci&oacute;n',
                    msg: 'No puede asignar el responsable a este plan de accion',
            ...
            if (item.data.estatus == 'Sin Plan' || item.data.estatus == 'Pendiente') {
                ...
                win.down('button[name=btnCambiar]').setVisible(false);
            }
        }
        win.show();
    ...
    onClickGuardarResponsable: function (button, e, options) {
        if (nuevorespo) {
            var grid = this.getListaEventoResponsable();
            var gridUsu = this.getListaAsignarUsuarioEncargado();
            var win = this.getWinAsignarEncargado();
